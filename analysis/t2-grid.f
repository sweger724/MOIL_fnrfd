...
c 
	...
	parameter(mesh=30)
        ...
        real grid_size
	real tgrid(mesh,mesh,mesh),t2grid(mesh,mesh,mesh)
        real v2
...
c
     ...
     1           .and.ptnm(j).eq.'NH1') then
            ...
            t2grid(ix,iy,iz) = t2grid(ix,iy,iz) + ptms(j)*ptms(j)*v2*v2
           ...
           do j=1,mesh
            do k=1,mesh
             if (ibox(i,j,k).eq.0) then
              ...
              t2grid(i,j,k) = t2grid(i,j,k)/ibox(i,j,k)
              t2grid(i,j,k) = t2grid(i,j,k)/0.002/0.002
              t2grid(i,j,k) = t2grid(i,j,k)-tgrid(i,j,k)*tgrid(i,j,k)
              t2grid(i,j,k) = sqrt(t2grid(i,j,k))
             end if
             write(utemp,*)i,j,k,t2grid(i,j,k)
             if (t2grid(i,j,k).gt.tcut) then
              ncount = ncount + 1
              coor(1,ncount) = xmin + (i-1)*dx
              coor(2,ncount) = ymin + (j-1)*dy
              coor(3,ncount) = zmin + (k-1)*dz
              more(ncount) = t2grid(i,j,k)
