...
c rms_ave: rms of dynamic structrue respect to averaged dynamic structure
            integer ucon,urdcrd,uwdcrd,uwflu,urcrd,uwrms,uwrms_ave
            ...
            double precision tmp_mass(maxpt)
            character*11 name
            logical find,fopen,fluct,rmsav,rmsre
            logical pickpt
            double precision Tr,cosine
...
5         continue
             if (.not. fopen(ucon)) then
               ...
               call alert(name,namel,'urdcrd not opened',16,level)
                else if (.not. fopen(uwdcrd)) then
               level=1
               call alert(name,namel,'uwdcrd not opened',16,level)
...
c   read dynamics coordinates
             ...
             do 85 k=1,n,jump
              ...
              if (rmsre)then
                ...
                call rmsd_weight(iselect,coor2,coor,rms,.false.
     1             ,tmp_mass)
...
c   write new dynamic coor-s

                call wdyncrd(uwdcrd,1,k,inofrz,nofreez,wbin)
                t=k
