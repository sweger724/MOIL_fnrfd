...
c
        if (.not.fopen(ucon)) then
         ...
         call alert(name,namel,'urvel not opened',16,level)
          ...
          do l=lpstr,lpend
           ...
           do j =1,npt
            if (moname(poimon(j)).ne.'TIP3') then
            x = coor(1,j)
            y = coor(2,j)
            z = coor(3,j)
            ix = (x-xmin)/dx+1
     ...
     1           .and.ptnm(j).eq.'NH1') then
            ...
            tgrid(1,ix,iy,iz) = tgrid(1,ix,iy,iz) + velo(1,j)*ptms(j)*v2
            tgrid(2,ix,iy,iz) = tgrid(2,ix,iy,iz) + velo(2,j)*ptms(j)*v2
            tgrid(3,ix,iy,iz) = tgrid(3,ix,iy,iz) + velo(3,j)*ptms(j)*v2
            end if
           end do
          end do
          ncount = 0
           ...
           do j=1,mesh
            do k=1,mesh
             if (ibox(i,j,k).eq.0) then
              ...
              tgrid(1,i,j,k) = tgrid(1,i,j,k)/(0.002*ibox(i,j,k))
              tgrid(2,i,j,k) = tgrid(2,i,j,k)/(0.002*ibox(i,j,k))
              tgrid(3,i,j,k) = tgrid(3,i,j,k)/(0.002*ibox(i,j,k))
             end if
             write(utemp,*)i,j,k,tgrid(1,i,j,k),tgrid(2,i,j,k)
     1            ,tgrid(3,i,j,k)
             if (tgrid(1,i,j,k).gt.-999) then
             if (abs(tgrid(1,i,j,k)).gt.tcut .or. 
     1           abs(tgrid(2,i,j,k)).gt.tcut
     1           .or. abs(tgrid(3,i,j,k)).gt.tcut) then
              ncount = ncount + 1
              poimon(ncount) = ncount
              coor(1,ncount) = xmin + (i-1)*dx
              coor(2,ncount) = ymin + (j-1)*dy
              coor(3,ncount) = zmin + (k-1)*dz
              more(ncount) = tgrid(1,i,j,k)*tgrid(1,i,j,k)+
     1        tgrid(2,i,j,k)*tgrid(2,i,j,k)+
     1        tgrid(3,i,j,k)*tgrid(3,i,j,k)
              ptnm(ncount) = 'XE'
              moname(ncount) = 'XE'
             end if
             end if
