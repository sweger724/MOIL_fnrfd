...
C               write(*,*)' i j k coor(i) coor(j) coor(k) '
               !write(*,*)i,j,k
C               write(*,*)(coor(l,i),l=1,3)
C               write(*,*)(coor(l,j),l=1,3)
C               write(*,*)(coor(l,k),l=1,3)
               !write(*,*)' angleq kangl ',angleq(ith),kangl(ith)
...
!c mauro - <U1-U2>l
     ...
     &                                    .or. mutaid(k).eq.2) then
                           tmp_e_lambda=tmp_e_lambda-e/(1.d0-lambda)
                        endif
!c end mauro - <U1-U2>l
                e_theta = e_theta + e
C@
               !write(*,*)' e e_theta ',e,e_theta,at
                df = df+df
                at = dsin(at)
...
c               by assumption no angles between slow and fast
