...
c     ---------------------------------
      ...
      if (eCGyes) then
        call eCG()
        e_total = e_total + e_CG
        if (efyes) then
            call nbfinitCG()
        end if
      endif

      if (e_Kumbyes .and. my_pe .eq. num_pes-1) then
          if (Nreduced .gt. 0) then
            call angle_constraint()
          else
            call interface_constraint()
          end if
          e_total = e_total + e_Kumb
      end if
      ...
      if ( rgcst ) then
       ...
       if (rwall) then
        e_rwall=0.d0
        call repulsive_wall()
        e_total = e_total + e_rwall
       endif
 
       if (efield_yes) then
        e_efield = 0.d0
        call e_field()
        e_total = e_total + e_efield
       endif
       if (esteeryes) then
        e_steer = 0
        call steer()
        e_total = e_total + e_steer
       end if
123     continue
