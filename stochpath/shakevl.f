...
c @ NO NEED FOR ptms IN STO. ALL MASSES ARE SET UNIFORMLLY TO 1
c
...
c
      ...
      double precision fac(maxshak)
      double precision ovrlx,mass1,mass2
...
C          check for convergence of all distances and return if ok
          do 200 k=1,nshak
              ...
              vry = velo(2,iat1) - velo(2,iat2)
              vrz = velo(3,iat1) - velo(3,iat2)
c	      write(*,*)' iterate iat1 iat2 ',iterate,iat1,iat2
c	      write(*,*)' SHAKVL dabs(diff) epsilon ',dabs(diff),epsilon
              diff = (vrx*facx(k)+vry*facy(k)+vrz*facz(k))
	      if (dabs(diff).gt.epsilon) then
c		write(*,*)' iterate iat1 iat2 ',iterate,iat1,iat2
c		write(*,*)' vrx,vry,vrz ',vrx,vry,vrz
c		write(*,*)' SHAKVL dabs(diff) epsilon ',dabs(diff),epsilon
c		write(*,*)' cooref=',cooref(1,1),cooref(1,2),cooref(1,3)
c chen
...
1000	  continue
	  level = 1
      call alert('shakevl',7,' Velocity shake not converged',29
     6  ,level)
	  return
	  end
