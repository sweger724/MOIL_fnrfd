...
C     p() carries our momenta of each sys.
      ...
      double precision dv(3,MAXPT*LGRID),dv_tmp(3,MAXPT*LGRID) 
...
C       Calculate the energies and forces
        ...
        call Get_EnergyDerivatives(pseg,npt,r,dv,e0)
...
C        d.../dx(j) = dv(j)/(ENERGYC-e0(i))
        do j = 2,pseg+1
          ...
          do i = 1,npt
            if (ipick(i).gt.0) then
              do loop=1,3
                dsall(loop,i+k) = dsall(loop,i+k) + e1(j)*dv(loop,i+k)
              end do
            end if
         end do
        enddo
...
C     Now calculate dS
              call Get_dS(pseg,npt,r,dv,p,d0,pdqA,dS)                  
...
C       then call Get_dS ...
              call Get_EnergyDerivatives(pseg,npt,r,dv,e_tmp)         
C             do i = 2,pseg+1
...
C             end do
              ...
              call Get_dS(pseg,npt,r,dv,p_tmp,d0_tmp,pdqA,dSp)
...
C*********************************************************************
        subroutine Get_EnergyDerivatives(pseg,npt,rr,dv,e0)
C*********************************************************************
        implicit none

        double precision rr(3,*),dv(3,*),e0(*)
        integer pseg, npt
...
C                write(6,*)"rr",rr(loop,i+k)
 ...
 55      continue
         ...
         dls(j) = dsqrt(dls(j))
C         write(6,*)"dls(j) ",j,dls(j)/dsqrt(allmass)
 56   continue
