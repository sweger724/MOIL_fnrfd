...
c     stdo - where some information on the path is printed out
      ...
      double precision kphp,kpsp,keqp,kphn,kpsn,keqn
...
c     pre-equilibration temperature
      ...
      integer ucon,umlst,ufpt,ufpp,utrc
      integer nwcrd,uwcrd,uwpep,urcrd,nrcrd,uene,umom
...
c     friction coefficient for brownian dynamics. I am aware
c     that DYNA.BLOCK defines LDgamma, but when I try to
c     include it, the compiler complains. -TF 21-May-2004
...
c     torsion constraint params (TF, 25-May-2005)
      kphi = 0.d0
      kpsi = 0.d0
      keq = 0.d0
      ctstr = 0.d0
...
c      else if ( .not. fopen(uwpep) ) then
c         level = 1
c         call alert(name,namel,'uwpep not opened',16,level)
...
c     internally the 1-4 scaling factors are defined as the inverse
      ...
      write(stdo,101) dt,ntest,gamma,irand,nwcrd,ptemp,neqstep,nsvel
      write(stdo,102) kphi,kpsi,keq,ctstr,kphn,kpsn,keqn,kphp,kpsp,keqp
...
c     In imitation of the previous line, I'll scale gamma using
c     tfac instead of tconv. -TF 21-May-2004.
      gamma = gamma * tfac
...
c     LEFT (previous) MILESTONE
      if (pmlst .ge. 1) then
         call getmlst(umlst,plp,pln,pmlst,num_mlsts,pointr,npick)
      end if
      
c     RIGHT ('next') MILESTONE
      if (nmlst .ge. 1) then
         call getmlst(umlst,prp,prn,nmlst,num_mlsts,pointr,npick)
      end if
...
c         call wmlst(utrc,plp,pln,npick,trceps)
         call wmlst(utrc,pmp,pmn,npick,trceps)
c         call wmlst(utrc,prp,prn,npick,trceps)
     ...
     3     grdlx,grdly,grdlz,
     4     dpold,fact11,fact22,stdo,urcrd,ufpt,ufpp,uwcrd,uwpep,uene,
     5     nwcrd,ntest,ptemp,neqstep,newv,irand,debug,scalar,sigma,
     6     nlist,massw,shakm,shakb,shakl,
     7     prp,prn,plp,pln,pmp,pmn,orth,tfp,
     8     kphn,kpsn,keqn,kphp,kpsp,keqp,
     9     umom,ucrb)
