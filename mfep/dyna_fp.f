...
c           ensemble.
            if ( iens .eq. 1 ) then
               ...
               if ((mod(istp,nwcrd) .eq. 0) .and. (istp .gt. nequ)) then
                  ...
                  if ( udot1 .gt. 0 ) write(udot1,*) dotl, dotm, dotr
...
!                    write(6,*) "KKK",k,pointr(k),tmp_mass(pointr(k))
                  ...
                  call rmsd_weight(npt,coorstart,dummy,rms,.false.)
...
c     write dot products
         if ( ( udot .gt. 0 ) .and. ( orth .or. tfp ) ) write(udot,
