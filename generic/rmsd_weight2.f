        subroutine rmsd_weight2(Npart,npick2,coor,coor2,rms,
     1centerCoor,pick,pick2)
        implicit none
c
...
c
        ...
        double precision coor(3,*),coor2(3,*),pick(*),pick2(*)
        logical centerCoor
        double precision rms
        integer Npart,npick2
...
c calculate current rms
        ...
        if (Npart .ne. npt) then 
          ...
          do i=1,Npart
            ...
            rmsarray(i) = pick2(i)*( tmp(1)**2 + tmp(2)**2 + tmp(3)**2 )
            rms = rms + rmsarray(i)
            allmass= allmass + pick2(i)
          ...
          write(*,*) ' in else '
          do j=1,npt
          if (pick2(j).gt.0.1) write(*,*)' j pick2(j)',j,pick2(j)
          end do
          write(*,*)' totmon = ',totmon
          do j=1,totmon
            rmsarray(j) = 0.d0
            ...
            do i=poipt(j-1)+1,poipt(j)
              if (pick2(i).gt.0.0001) then
             write(*,*)' poipt1 poipt2 ',poipt(j-1)+1,poipt(j)
              tmp(1) = coor2(1,i) - coor(1,i)
              tmp(2) = coor2(2,i) - coor(2,i)
              tmp(3) = coor2(3,i) - coor(3,i)
              if (tmp(1).gt.1000 .or. tmp(2).gt.1000
     &           .or. tmp(3).gt.1000) then
                 write(*,*)' i tmp1-3 ',i,tmp
              end if
              rmsarray(j) = rmsarray(j) + pick2(i)*(tmp(1)**2
     $      + tmp(2)**2 + tmp(3)**2 )
              wmonmass = wmonmass + pick2(i)
              allmass = allmass + pick2(i)
              if (j.eq.3. .or. j.eq.4 .or. j.eq.8) then
               write(*,*)'i tmp1-3 ',i,tmp
               write(*,*)' pick2 ',pick2(i)
              write(*,*)' rmsarray(',j,') ',rmsarray(j)
              end if
              end if
            end do 
            write(*,*)' j wmonmass rms rmsarray(j) ',
     &      j,wmonmass,rms,rmsarray(j)
            if(wmonmass.gt.0.01d0) then
            write(*,*)' wmonmass rms rmsarray(j) '
     &       ,wmonmass,rms,rmsarray(j)
              rms = rms + rmsarray(j)
