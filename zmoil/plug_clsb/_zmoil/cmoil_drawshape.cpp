...
extern int Zmoil_PeptidePlaneFlipping;
extern int Zmoil_PeptidePlaneWireframe;
extern int Zmoil_DrawSheets;
extern float Zmoil_VelocityScale;
extern int Zmoil_VelocityCAOnly;
extern  float Zmoil_VelocityMin;
...
const DiskArray Tube::UnitCircle = {
	{TubeRadius, 0., 0.},
...
void SecStruct::Draw(  ) {
	...
	if( !Zmoil_DrawSheets && ( secStructLow == STRUCT_SHEET || secStructLow == STRUCT_LASTOFSHEET ) ) {
		secStructLow = STRUCT_UNKNOWN;
	}
	...
	if( ( secStructLow == STRUCT_LASTOFSHEET ) && bss[0].numOut > 1 ) {	// start an arrow, modify width
	...
	if( bss[0].numOut == 1 ) {
	...
	else if( sIsHelix( secStructLow )
			 /* && (secStructLow!=oldStructLow) */
		 ) {
		...
		if( nhelix < 2 )	// check helix's normal
		{
			mid = bss[0].numOut / 2;
			if( ( normal
				  ( lastDraw[0], bss[1].a_out[mid],
					bss[0].a_out[mid] ) ^ ( bss[0].a_out[mid] - avg( lastDraw[0],
																	 bss[0].a_out[bss[0].numOut - 1] ) ) ) < 0 )
				direction = -1;
			else
				direction = 1;
		}
...
void SecStruct::StructEndBegin( SecStructType oldStruct, SecStructType newStruct ) {
	...
	if( sIsUnknown( newStruct ) || sIsUnknown( oldStruct ) ) {
		...
		if( sIsUnknown( newStruct ) ) {
			...
			glNormal3d( 0, 0, 0 );
			if( bss[0].numOut > 1 ) {	// repeat last point as the beginning of new struct type
...
SplineBelt3D::SplineBelt3D( AtomArray * aaptr, int windowSize,
							int colorfilter ):SplineShape( aaptr, windowSize, colorfilter, 2 ) {
...
void SecStruct3D::DrawDo(  ) {
	...
	while( ii < bsplineOutCount ) {
		...
		if( sIsHelix( bsplineOutStruct ) )
			DrawHelix( ii, jj );
		else if( Zmoil_DrawSheets && sIsSheet( bsplineOutStruct ) )
			DrawSheet( ii, jj );
		else
			DrawLoop( ii, jj + 1 );
		ii = i;
...
SecStruct3D::SecStruct3D( AtomArray * aaptr, int windowSize, int colorfilter ):SplineBelt3D( aaptr, windowSize,
																							 colorfilter ) {
	oldStructLow = STRUCT_UNKNOWN;
	bsplineOutStruct = STRUCT_UNKNOWN;
	prevStruct = STRUCT_UNKNOWN;
...
void SplineShape::setColorBase( tRGBA * color ) {
	colorbase = color;
}

// if colorsetting=NULL, then use the defaultColor
extern unsigned char gGlobalAlpha;
void SplineShape::setColor( int rn, tRGBA * defaultColor ) {
	tRGBA *c = ( colorsetting == NULL ) ? defaultColor : ( colorsetting + rn );
	if( c != NULL ) {
		glColor4ub( Color2R( colorFilter, c ), Color2G( colorFilter, c ), Color2B( colorFilter, c ), gGlobalAlpha );
	}
}
...
SplineShape::SplineShape( AtomArray * aaptr, int windowsize, int colorfilter, int numbss ):numBss( numbss ) {
	...
	if( Zmoil_PeptidePlaneWireframe ) {
...
void AtomArray::backbone( int windowSize, int colorFilter, int type ) {
	if( DrawBackboneAsCurve )
		backboneCurve( windowSize, colorFilter, type );
	else
		backboneTube( windowSize, colorFilter, type );
}

void AtomArray::backboneCurve( int windowSize, int colorFilter, int type ) {
	Curve curve( this, windowSize, colorFilter );
	if( type == RNABACKBONE ) {
		curve.setColorBase( &orange );
	}
	int index;
	if( (index=properties.getI( "colorIndex", -1 )) != -1 ) {
		curve.setColorBase( &zmoilColors[ index ] ); 
	}
	backbone( &curve, windowSize, colorFilter, type );
}

void AtomArray::backboneTube( int windowSize, int colorFilter, int type ) {
	Tube tube( this, windowSize, colorFilter );
	if( type == RNABACKBONE ) {
		tube.setColorBase( &orange );
	}
	int index;
	if( (index=properties.getI( "colorIndex", -1 )) != -1 ) {
		tube.setColorBase( &zmoilColors[ index ] ); 
	}
	backbone( &tube, windowSize, colorFilter, type );
}

void AtomArray::backbone( SplineShape * Loop, int windowSize, int colorFilter, int type ) {

	// Draw a spline-based backbone between the backbone atoms whose type is determined
	// by the "type" parameter which can be { BACKBONE, RNABACKBONE }

	const char *backboneAtomID = "CA";
	if( type == RNABACKBONE ) {
		backboneAtomID = "P";
	}

	int  inNucleicAcid = !strcmp( atomList[0].atomName, "H5T" ) || !strcmp( atomList[0].atomName, "OP3" );
		// The molecule may be a nucleic-acid/protein complex - so we track which
		// portion we are inside of by looking for markers like H5T,H3T and HX2
		// 01.11.10: added the OP3 which is the atom name in the first residue of PDB rna files.

	Atom *listi = atomList + firstVisibleAtom, *previ = atomList + firstVisibleAtom;
	Atom *lastCA = 0;
	Loop->ChainBegin();
	int connectedLast = 0;
	int lastWasSkipped=1;
	for( int I = firstVisibleAtom; I < numAtom && I <= lastVisibleAtom; I++, listi++ ) {
		if( !inNucleicAcid ) {
			// check to see if we are entering a nucleic acid portion (1.11.10: added OP3 for pdb files)
			if( !strcmp( listi->atomName, "H5T" ) || !strcmp( atomList[0].atomName, "OP3" ) ) {
				inNucleicAcid = 1;
			}
		}
		else {
			// check to see if we are entering a protein portion
			if( !strcmp( listi->atomName, "HX2" ) ) {
				inNucleicAcid = 0;
			}
		}
		int rnaEnd = inNucleicAcid && !strcmp( listi->atomName, "H3T" );
		int proteinEnd = ( !inNucleicAcid && 
						   previ->rn != listi->rn && sIsChainEnd( previ->secStruct ) );
		int sectionEnd = !lastWasSkipped && listi->skip;
		lastWasSkipped = listi->skip;
		if( proteinEnd || rnaEnd || sectionEnd ) {
			if( lastCA ) {
				Loop->Draw( lastCA );
				Loop->Draw( lastCA );
				connectedLast = 1;
				// tfb: force the spline to end at the last alpha-carbon atom, otherwise
				// the spline will stop short, awaiting further points.
			}
			Loop->ChainEnd(  );
			Loop->ChainBegin(  );
			connectedLast = 0;
		...
		if( !listi->skip ) {
			if( !strcmp( listi->atomName, backboneAtomID ) ) {
				Loop->Draw( listi );
				lastCA = listi;
			}
...
void AtomArray::quadStripStruct( SplineShape * splineShape, int windowSize, int colorFilter, bool isRibbon ) {
	Atom *listi = atomList + firstVisibleAtom, *currRes = atomList + firstVisibleAtom;
...
void AtomArray::peptidePlaneVectors() {
	Atom *listi = atomList, *currRes = atomList;
	Coordinate currCA, currO, lastCA, lastO;
	Coordinate A, B, C, D;
	Atom *currCA_atom=0;
	...
	*/
	...
	int i;
	int skipVectors=1;
	for( i = 0; i < numAtom; i++, listi++ ) {
		if( currRes->rn != listi->rn )	// new residue
		{
			if( foundCA && foundO && !currCA_atom->skip ) {
				if( skipVectors ) {
					skipVectors=0;
						// don't draw first set of vectors since "lastCA" etc will be at origin
				}
				else {
					...
					glColor3ub( 255, 0, 0 );	// ca to ca is red
					zglDrawVector( lastCA.x, lastCA.y, lastCA.z,
								   A.x, A.y, A.z );

					glColor3ub( 0, 255, 0 );	// ca to o is green
					zglDrawVector( lastCA.x, lastCA.y, lastCA.z,
										B.x, B.y, B.z );

					glColor3ub( 0, 0, 255 );	// normal to plane is blue
					zglDrawVector( lastCA.x, lastCA.y, lastCA.z,
								   C.x, C.y, C.z );
				}
			}
	...
	for( i = 0; i < numAtom; i++, listi++ ) {
		if( currRes->rn != listi->rn )	// new residue
		{
			if( foundCA && foundO && !currCA_atom->skip ) {
				...
				glVertex3dv( &currCA.x );
				glVertex3dv( &lastCA.x );
				glVertex3dv( &lastO.x );
			}
			currRes = listi;
			foundCA = false;
...
void AtomArray::velocityVectors() {

	double maxVel = properties.getD( "velocityMax", 1.0 );
	double minVel = properties.getD( "velocityMin", 0.0 );

	int i;
	Atom *listi = atomList;
	for( i = 0; i < numAtom; i++, listi++ ) {
		if( !listi->skip && (!Zmoil_VelocityCAOnly || !strcmp( listi->atomName, "CA" )) ) {
			double mag = listi->velocityMag();
			if( mag > Zmoil_VelocityMin ) {
				double magScaled =  mag / maxVel;
				DVec3 vel(listi->vx, listi->vy, listi->vz );
				vel.normalize();
				vel.mul( magScaled * Zmoil_VelocityScale );

				FVec3 color;
				getSpectralRGB( 1.0 - magScaled, color );
				glColor3fv( color );

				zglDrawVectorDst( listi->x, listi->y, listi->z, listi->x + vel.x, listi->y + vel.y, listi->z + vel.z );
	...
	for( i = 0; i < numAtom; i++, listi++ ) {
		if( !listi->skip && (!Zmoil_VelocityCAOnly || !strcmp( listi->atomName, "CA" )) ) {
			...
			if( mag > Zmoil_VelocityMin ) {
				...
				glVertex3f( listi->x, listi->y, listi->z );
			}
		}
	}
	glEnd();
