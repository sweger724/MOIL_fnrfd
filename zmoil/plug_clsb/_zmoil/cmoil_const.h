...
enum DisplayMode 				// for both overall & single atom's display mode
{
  ...
  RAVINORM=0x400,
  RAVINORMCOARSE=0x800,
  RESIDUEPICK=0x1000,
  RNABACKBONE=0x2000,
  VELOCITY_COLOR=0x4000,
  VELOCITY_VECTOR=0x8000,
...
const unsigned short int ATOM_MAX_HBONDS = 8;	// max number of hydrogen bonds per atom

const unsigned short int ARRAY_MAXNUM	  =	128;	// maximum number of input pictures

const double PI = 3.14159265359;					// Pi

const float PI10th=(float)(PI/10.0);
const float PI2DEGREE=(float)(180.0/PI);
...
public:
  ...
  //tRGBA(tRGBA &copy) { set( copy.r, copy.g, copy.b ); }
  void set(GLubyte rm, GLubyte gm, GLubyte bm) { r=rm;g=gm;b=bm;setGrey(); };
  void set(tRGBA &copy) { r=copy.r; g=copy.g; b=copy.b; setGrey(); };
  void setGrey(void) { w=(GLubyte)Color2Mono(r,g,b); } ;
