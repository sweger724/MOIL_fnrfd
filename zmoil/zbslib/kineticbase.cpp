...
void KineticExperiment::simulate( struct KineticVMCodeD *vmd, double *pVec, int steps ) {
	...
	for( i=0; i<mixstepCount; i++ ) {
		...
		if( steps > 0 ) {
			...
			integrator->integrateD( i==0 ? ic : dilutedC, rrPlusOtherParams, endTime, startTime );
		}
		zprofEnd(); // integrateD 
	...
	if( isDependent( DT_Volt ) || isDependent( DT_Temp ) ) {
		...
		if( isDependent( DT_Temp ) ) {
			for( int r=0; r<reactionCount(); r++ ) {
				...
				info.type = PI_TEMPERATURE_COEF;
				info.reaction = r;
				strcpy( info.name, ZTmpStr( "%s_Ea", reactionGetRateName( r ) ) );
...
void KineticSystem::simulate( int forceAllExp /* =1 */, int steps /* =-1 */) {
    ...
    if( isDependent( KineticSystem::DT_Temp ) ) {
        // In the context of fitting we may need to update the Arrhenius equation amplitudes
        // to constrain them based on kinetic rates determined at a reference temperature.
        #ifdef USE_KINFIT_V2
        updateTemperatureAmplitudesFromReferenceFit();
        #endif
    }
...
int KineticIntegrator::integrateD( double *icC, double *P, double endTime, double startTime, double stopIfStepSmallerThan ) {
	assert( vmd->isCompiled() );
	int reagentCount = vmd->system->reagentCount();
