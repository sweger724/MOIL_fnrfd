	subroutine shakefr(epsilon,maxit)
c
...
C          adjust each constraint in order
          do 100 k=istart,nshak
              iat1 = ishak1(k)
              iat2 = ishak2(k)
              vrx = dpot(1,iat1) - dpot(1,iat2)
              vry = dpot(2,iat1) - dpot(2,iat2)
              vrz = dpot(3,iat1) - dpot(3,iat2)
...
C                  update coordinates along original difference vector
               dpot(1,iat1) = dpot(1,iat1) +  fac1 * cooref(1,k)
               dpot(2,iat1) = dpot(2,iat1) +  fac1 * cooref(2,k)
               dpot(3,iat1) = dpot(3,iat1) +  fac1 * cooref(3,k)
               dpot(1,iat2) = dpot(1,iat2) +  fac2 * cooref(1,k)
               dpot(2,iat2) = dpot(2,iat2) +  fac2 * cooref(2,k)
               dpot(3,iat2) = dpot(3,iat2) +  fac2 * cooref(3,k)
100       continue
c chen
...
1000	  continue
	  level = 1
       call alert('shakevl',7,' Forces shake not converged',29,level)
	  return
	  end
