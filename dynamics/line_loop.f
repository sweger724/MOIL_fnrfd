...
6             continue
         ...
         if (find('stee')) then
            ...
            kz  = getd('kz',kz)
            write(*,*)' npt = ',npt
            call pick(pick_steer,i)
            nsteer = 0
            do i=1,npt
            if (pick_steer(i).ne.0) then
             nsteer = nsteer + 1
             pick_steer_compress(nsteer) = i
            end if
            end do
          end if
         ...
         LDgamma   = getd('gama',LDgamma)  ! Langevin dynamics friction coef.
         ...
         newv    = geti('newv',newv)
         dt      = getd('step',(dt))
         nswap   = geti('swfr',nswap)  ! parallel tempering parameters
         Nreplicas = geti('Nrep',Nreplicas)
c

c mauro add for muta
         if (find('muta')) then
          muta      = .TRUE.
          in_lambda = 0.2d0 ! initial value of lambda in muta thermo integration
          in_lambda = getd('ilam',in_lambda)
          f_lambda  = 0.8d0 ! final value of lambda
          f_lambda  = getd('flam',f_lambda)
          n_step_lambda = 20 ! numbr of steps from initial to final
          n_step_lambda = geti('#lam',n_step_lambda)
          twait     = 1000 ! time to wait before average
          twait     = geti('twai',twait)
          if (find('banf')) BandF=.TRUE. ! ilam -> flam -> ilam? T ilam -> flam? F
          if (2*n_step_lambda+1 .ge. lambda_max) then
            level=1
            call alert(name,namel,'lambda_max small',16,level)
          endif
         endif
...
c Urey-Bradley
        if (find('UREY')) then
         nurey = nurey + 1
         urey1(nurey) = geti('ang1',0)
         urey2(nurey) = geti('ang2',0)
         urey3(nurey) = geti('ang3',0)
         write (stdo,*) 'u-b angle ',nurey,'between',urey1(nurey),
     1                   urey2(nurey),urey3(nurey)
        endif
...
c
        if (find('ball')) then
                ...
                write(*,1001)fball,rball,rcball
1001            format(1x,'Current valuse for fball rball rcball '
     1            ,3(1x,f9.3))
        end if
...
C SWITCH IS NOT SET UP FOR PARALLELIZATION (YET)        
        ...
        photon_per_step = getd('phot',photon_per_step)
        delt  = getd ('delt',delt )
...
106               format(1x,' Walls are located at +/- ',f9.3,
     11x,' The Electrode potential is ',f10.4)
       end if
...
c
c compute temperature for a slected set number of atoms
c RE 1/14/2011
c
	if (find('tsel')) then
		Ltpick = .true.
