...
#else
	#include "GL/glut.h"        // for GL_FILL & GL_LINE
#endif
#include <stdlib.h>
#include <ctype.h>
...
InputParameters::InputParameters () 
{
   ...
   filetag     = fNONE; 
   sleepseconds=0.0; 
   structno   = -1;			// tfb: this was uninitialized; if init to >= 0, surf.exe needs to run
   structno  = -1;			// tfb: this was not being initialized at all. Not sure if -1 or 0 more appropriate.  0 causes surf.exe to run
   ...
   dispmode = STICK;          // STICK;  Show stick always
   bondmode = 0;
   swapmode = 0;
   ...
   rotateDegree=5.0*AngleToRadius;         // 5.0 degree for each button click by default
   subsurf=NULL;
   subsurfLen=0;
   transparent=255;           // 
   ...
   cutoffval=0;
   cutoffstep=2.0;
   surfprobe=1.4;            // default radius : water=1.4
...
int InputParameters::ReadInput( char *inputfile, int crdIndex, char *cmoil)
{
  ...
  while (fgets(s,LINE_MAXLEN,fp) != NULL)
  {
	...
	sscanf(s, "rotatedegree=%f", &rotateDegree);
      rotateDegree *= AngleToRadius;
    }
    else if(strstr(s, "cavityonly="))
    {
      showCavity = atoi(s+11)==0?false:true;
      cutoffval = 1.e+20;                                          // no cutoff by default
    }
  ...
  {
    if ( connName != NULL && filetag != fXYZ && filetag != fPDB )
  ...
  if ( crdIndex > currCrd || (crdIndex > currWcon && filetag != fXYZ && filetag != fPDB ))	// missing crd or wcon file
    return -1;
  else
    return crdIndex;
