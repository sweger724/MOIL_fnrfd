...
{
   static float line[8];
   line[0] = pt2.y-pt1.y;    // A1
   line[1] = pt1.x-pt2.x;    // B1
   line[2] = 0.;             // C1
   line[3] = line[0]*pt1.x + line[1]*pt1.y;   // D1

   line[4] = 0.;             // A2
   line[5] = pt2.z-pt1.z;    // B2
   line[6] = pt1.y-pt2.y;    // B
   line[7] = line[5]*pt1.y + line[6]*pt1.z;   // D
   return line;
}
...
void Camera::LookAt(int leftRight, int stereomode)
{
	...
	eye = leftRight;        // left, middle, right
      ...
      glPopMatrix();
      glLoadIdentity();
      left  = - ratio * wd2 - eye * eyesep * ndfl;
      right =   ratio * wd2 - eye * eyesep * ndfl;
      top    =  wd2;
      bottom = -wd2;
