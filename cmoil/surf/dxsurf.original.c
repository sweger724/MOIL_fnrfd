...
int refinter, hashdim, nohash, *hashtable, *hash_index_list, natoms;
...
static Error DoSurf(Object o,float probe, float gridres,int colorflag) {
    ...
    DXMessage("extracted components");
...
/* set actual number of positions  for trim */
  ...
  DXAddArrayData(nb,0,Num_polys,NULL);
...
for (i=0; i<Num_polys; ++i) {
  ...
  poss[9*i  ] = verts[i][0].Coord[X];
  poss[9*i+1] = verts[i][0].Coord[Y];
  poss[9*i+2] = verts[i][0].Coord[Z];

  poss[9*i+3] = verts[i][1].Coord[X];
  poss[9*i+4] = verts[i][1].Coord[Y];
  poss[9*i+5] = verts[i][1].Coord[Z];

  poss[9*i+6] = verts[i][2].Coord[X];
  poss[9*i+7] = verts[i][2].Coord[Y];
  poss[9*i+8] = verts[i][2].Coord[Z];

  nrml[9*i  ] = verts[i][0].Normal[X];
  nrml[9*i+1] = verts[i][0].Normal[Y];
  nrml[9*i+2] = verts[i][0].Normal[Z];

  nrml[9*i+3] = verts[i][1].Normal[X];
  nrml[9*i+4] = verts[i][1].Normal[Y];
  nrml[9*i+5] = verts[i][1].Normal[Z];

  nrml[9*i+6] = verts[i][2].Normal[X];
  nrml[9*i+7] = verts[i][2].Normal[Y];
  nrml[9*i+8] = verts[i][2].Normal[Z];

  ndata[9*i  ] = atom_type[i];
  ndata[9*i+1] = atom_type[i];
  ndata[9*i+2] = atom_type[i];

  ndata[9*i+3] = atom_type[i];
  ndata[9*i+4] = atom_type[i];
  ndata[9*i+5] = atom_type[i];

  ndata[9*i+6] = atom_type[i];
  ndata[9*i+7] = atom_type[i];
  ndata[9*i+8] = atom_type[i];
...
refinter = 3*Num_polys; /* number of vertices */
...
if ((tmp0 > 1.0e-10) && (tmp1 > 1.0e-10) && (tmp2 > 1.0e-10)) {
   ...
   conn[3*i] = ind1; conn[3*i+1] = ind2; conn[3*i+2] = ind3;
...
/*  DXMessage("comparing with %d dist = %f",index,dist2); */
...
/*           DXMessage("comparing with %d new dist = %f",index,dist2);
           DXMessage("%f %f %f vs %f %f %f",x,y,z,pos[3*index],pos[3*index+1],
                       pos[3*index+2]); */
...
/*           DXMessage("adding 1 %f %f %f at %d",x,y,z,refinter); */
...
/*        DXMessage("adding 2 %f %f %f at %d",x,y,z,refinter);  */
