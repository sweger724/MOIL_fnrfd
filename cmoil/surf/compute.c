...
Atom_List        *Atom_Grid;
float            Voxel_Side=0.;
int              Num_Voxels[3]={0,0,0};
double           Max_Radius=0;
Vector           Bounding_Tetra[4];
double           Extents[3][2]={0,0,0,0,0,0};
...
/* REG initialize counters */
   ...
   /* allocate space for vertices and atoms */

    ALLOCMAT(verts, Max_Gp_Polys, 3, VertexType);
    ALLOCN(atom_type, int, Max_Gp_Polys);
...
/*       if (i == 8) {   REG */
       Current_atom = i;
       atom1_bw = Current_atom;           // --bwang
       atom2_bw = Current_atom;           // --bwang
  ...
  /*   }  */
    ...
    fprintf(stderr,"Total Triangles %d Total constraints %d\n",Num_polys, Tot_cons);
    fprintf(stderr,"Max Neighbors per atom %d Average Neighbors per atom %g\n", 
...
{ 
  ...
  { for( j = 0; j < 3; j++)
    { 
      ...
      /*printf("i=%d  %f %f %f %f\n",i,temp_min,temp_max,Extents[j][MINC],Extents[j][MAXC]);*/
  ...
  Voxel_Side = (total_radius + total_radius)/((float)Num_atoms);
  ...
  /* printf("%d %d %d %d\n", Num_Voxels[X], Num_Voxels[Y], Num_Voxels[Z], total_voxels); */
...
else { /* fprintf(stderr,"buried atom\n"); */ }  /* REG */
...
Vector		sol;
{
  ...
  { fprintf(stderr,"find_sol_cons() warning: sol is somewhat away from vertex (%lf).\n", min_lambda[2]);
/*    fflush(stdout);  */ /* REG */
  }
}     
