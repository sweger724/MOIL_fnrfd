      subroutine theta2_pcs()
c
c Calculate second derivative of angle energy.
c
...
c
c The second derivatives are stored in a condensed (but not MAXIMALLY
...
c 27*maxangl. It is compact since none of the elements is always zero. 
c It is not fully compressed since particles may appear more than once,
c since the storage is done according to the angle index and not 
c according to the particle index. In the present formulation of the 
c energy it is quite convenient.
c
      ...
      double precision delkjx,delkjy,delkjz,cost,sint2,sint
      double precision theta,scalar1,scalar2,aix,aiy,aiz
...
c.....initialize loop over angles
      do 100 ith=ifirst,ilast
...
c........maintain the pointer to the "real" vector
          ...
          cost   = rijrkj*r
          if (dabs(cost).ge.0.999d0) then
             write (6,*) 'ATOMS ',i,j,k,cost
             write (6,*) 'BAILING BECAUSE of linearish angle! ',cost
             write (6,*) 'Try running at cooler temperatures '
             stop
          end if
          ...
          sint2   = 1.d0/(1.d0 - cost*cost)
          sint    = dsqrt(sint2)
          theta   = dacos(cost)
          scalar1 = 2.d0*kangl(ith)*sint2*( 1.d0 -
     1           (theta-angleq(ith))*cost*sint)
          scalar2 = -2.d0*kangl(ith)*sint*(theta-angleq(ith))
