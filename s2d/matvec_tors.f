      subroutine matvec_tors(vector,vectout)
c 
...
c            mass = 1.0/( sqrt(ptms(itor1(iphi)) * ptms(itor2(iphi)) ) )
            mass = massfac(itor1(iphi)) * massfac(itor2(iphi))
         else
            mass = 1.0
         end if
...
c     k,m
         ...
         if (MASSWEIGHT.eq.1) then
c            mass = 1.0/( sqrt(ptms(itor1(iphi)) * ptms(itor3(iphi)) ) )
            mass = massfac(itor1(iphi)) * massfac(itor3(iphi))
         else
            mass = 1.0
         end if
...
c k,n
        ...
        if (MASSWEIGHT.eq.1) then
c           mass = 1.0/( sqrt(ptms(itor1(iphi)) * ptms(itor4(iphi)) ) )
           mass = massfac(itor1(iphi)) * massfac(itor4(iphi))
        else
           mass = 1.0
        end if
...
c l,m
        ...
        if (MASSWEIGHT.eq.1) then
c           mass = 1.0/( sqrt(ptms(itor2(iphi)) * ptms(itor3(iphi)) ) )
           mass = massfac(itor2(iphi)) * massfac(itor3(iphi))
        else
           mass = 1.0
        end if
...
c l,n
        ...
        if (MASSWEIGHT.eq.1) then
c           mass = 1.0/( sqrt(ptms(itor2(iphi)) * ptms(itor4(iphi)) ) )
           mass = massfac(itor2(iphi)) * massfac(itor4(iphi))
...
c m,n
        ...
        if (MASSWEIGHT.eq.1) then
c           mass = 1.0/( sqrt(ptms(itor2(iphi)) * ptms(itor4(iphi)) ) ) 
           mass = massfac(itor3(iphi)) * massfac(itor4(iphi))
        else
           mass = 1.0
        end if
