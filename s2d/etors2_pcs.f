      subroutine etors2_pcs()
c
c Calculate second derivative of torsion energy.
c
...
c
      ...
      double precision den,den2,den3,co,scalar1,scalar2
...
c        calculate multplicative constants
        if(.not.arith)then
         scalar2 = phase(iphi)*(12.d0*ktors3(iphi)*co*co
     1        +4.d0*ktors2(iphi)*co+ktors1(iphi)-
     2        3.d0*ktors3(iphi))
         scalar1 = phase(iphi)*(24.d0*ktors3(iphi)*co+
     1        4.d0*ktors2(iphi))
        else
           if(period(iphi).eq.1) then
            scalar2 = ktors2(iphi)*phase(iphi)
            scalar1 = 0.0d0
           else if(period(iphi).eq.2) then
            scalar2 = 4.0d0*ktors2(iphi)*phase(iphi)*co
            scalar1 = 4.0d0*ktors2(iphi)*phase(iphi)
           else if(period(iphi).eq.3) then
            scalar2 = ktors2(iphi)*phase(iphi)*(12.0d0*co*co-3.0d0)
            scalar1 = ktors2(iphi)*phase(iphi)*(24.0d0*co)
           else if(period(iphi).eq.4) then
            scalar2 = 16.0d0*ktors2(iphi)*phase(iphi)*co*
     &              (2.0d0*co*co-1.0d0)
            scalar1 = 16.0d0*ktors2(iphi)*phase(iphi)*
     &              (6.0d0*co*co-1.0d0)
           endif
          endif
         
c@
C@          write(*,*)' scalar1 scalar2 ',scalar1,scalar2
