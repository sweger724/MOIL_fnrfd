	...
	integer namel,level,k,kk,jj,ii
	integer of,i,j,ith,iith,kkk,jjj
	integer l,m,n,ll,mm,nn,iphi,iiphi
	integer jstart1,jend1,jstart2,jend2
	integer jstart3,jend3,jstart4,jend4
	logical find
...
22	continue
	...
	if (evdyes.or.eelyes) call cdie2()
	if (debug) then
...
45	continue
	...
	do 29 i=1,3*npt
	 write(stdo,100)(d2v(i,j),j=1,3*npt)
...
c for a test purposeds another way of recovering the second
c derivative matrix is by multiplying the matrix in the condesned form
c by a vector (0,...,0,1,0,0,0....,0) which should recover one
c coloumn by repeating it for different positions for the 1 all the
c matrix is calculated.

	write(stdo,*)
	write(stdo,*)' NEW GENERATION OF SECOND DERIVATIVE '
	do 32 i=1,3*npt
	 do 30 j=1,3*npt
	  vector(j) = 0.d0
30	 continue
	 vector(i) = 1.d0
	 call matvec(vector)
	 write(stdo,100)(vector(j),j=1,3*npt)
32	continue
	return
	end
