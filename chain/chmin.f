...
c
        ...
        integer geti,of, irand
        double precision getd, step
...
c       integration step in simulated annealing
        step    = 0.0005d0
...
c overlap all structures with respect to the middle one
        ...
        if (lap) then
         ...
         do i=1,igrid
          ix1 = (i-1)*npt3+1
          if (ix1.ne.imx1) call 
     &        rmsd_weight(npick,r(imx1),r(ix1),rms,.false.,.false.)
         end do
        end if
...
c
     ...
     3          nwcrd,ntest,debug,scalar,sigma,sigmav,gamma,
     4          rho,lambda,fixend,nlist,step)
        end if
