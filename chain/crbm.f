	Subroutine crbm(r,scalar,sigma,grdcmx,grdcmy,grdcmz,
     1	grdlx,grdly,grdlz,divms,npt,igrid,nselec,pointr,debug,
     2	udata)
c
...
c
	...
	integer pointr(*)
	integer npt,igrid,nselec,udata
	logical debug
c
c r          - coordinates
...
c$DOIT IVDEP
		do 4 j=1,nselec
		 ...
		 k = 3*(i-1)
		 r(k+1)=r(k+1)-divms(j)*(scalar(1)*grdcmx(j)
     1			+scalar(4)*grdlx(j)
