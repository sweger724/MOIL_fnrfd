        program dynamics
c  Molecular dynamics program with the usage of homology
...
c     proportion of homology energy
        ...
        call init_dyna()
c
c  initialize variables for parallel run
        call init_mpi_var()
...
c
         call eforce()
c       Each Hstep add a difference penalty to the energy term
        if (RUN_HOMOLOGY .AND. mod(j,Hstep).eq.0) then 
               call homology_force()
         end if
...
c calculate new forces
          call eforce()
c       Each Hstep add a differebce penalty to the energy term
          if (RUN_HOMOLOGY .AND. mod(j,Hstep).eq.0) then
                write(*,*),'Step:',j
                call homology_force()
          end if
...
C
          if (ncoor.ne.0 .and. my_pe.eq.0) then
           if (mod(j,ncoor).eq.0) then
              call wdyncrd(uwcrd,nstep/ncoor,j/ncoor,inofrz,nofreez)
           end if
          end if
