...
c
c local
        integer i,j,k,npt3,ndegf,smlp,imin,middle,l
        integer ncall
        ...
        if (eenmyes) call enm_lists(r(1,1),r(1,1+(igrid-1)*npt))
     ...
     5          fixend,finished)
          ...
          do 15 i=1,igrid
           if (i.le.igrid-2) then
            tmp1 = d0(i)
            call vecmin(r(1,(i-1)*npt3+1),r(1,(i+1)*npt+1),npt3,dv(1,1))
            tmp2 = vnorm(dv,npt3)
           ...
           write(udata,101)i,tmp1,tmp2,e0(i),e1(i)
15        continue
          tmp1 = 0.d0
          do 16 j=1,igrid-2
           k = j*npt
...
c
          ...
          write(udata,100)ncall,sener
100       format(1x,'chain energy after ',i6,' energy calls = ',f10.5)
          write(udata,*)' Stru # dist(i,i+1) dist(i,i+2) U  |grad(U)|'
          do 3 i=1,igrid
           if (i.le.igrid-2) then
            tmp1 = d0(i)
            call vecmin(r(1,(i-1)*npt+1),r(1,(i+1)*npt+1),npt3,dv(1,1))
            tmp2 = vnorm(dv,npt3)
           else if (i.le.igrid-1) then
            tmp1 = d0(i)
            tmp2 = 0.d0
           ...
           write(udata,101)i,tmp1,tmp2,e0(i),e1(i)
101        format(1x,i6,1x,4(f14.5,1x))
3         continue
          tmp1 = 0.d0
          do 4 j=1,igrid-2
           k = j*npt
