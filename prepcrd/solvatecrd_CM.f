...
c     zva : exclusion region from centered box
      double precision xbex,ybex,zbex,rvdw
...
c
      do 625 i = 1,nrec
         read(urwbx,'(20x,3f10.5)',end=630) xx,yy,zz
         read(urwbx,'(20x,3f10.5)') xx1,yy1,zz1
         read(urwbx,'(20x,3f10.5)') xx2,yy2,zz2
     ...
     &        .and.(abs(zz).lt.zb))) go to 625
         ...
         zz=zz-zb2
c     zva : start
         if (.not.((abs(xx).ge.xbex).and.(abs(yy).ge.ybex)
     &        .and.(abs(zz).ge.zbex))) go to 625
c     zva : end
         xx1=xx1-xb2
         yy1=yy1-yb2
         zz1=zz1-zb2
...
c
     ...
     >     ' zb=',zb
      write(stdo,'(//3x,a,i5//3x,a,i5/)') " natom=",natom,
