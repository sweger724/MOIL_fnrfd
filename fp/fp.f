...
c     boolean deciding between ucrb and phi/psi approaches
      ...
      data uene,utrc,upvl,udot,udot1,udst/6*99/
...
c        call wmlst(utrc,pmp,pmn,npick,trceps)
         ...
         call getmlst(umlst,pmp,pmn,tmlst,num_mlsts,npick,pointr)
         call getmlst(umlst,prp,prn,nmlst,num_mlsts,npick,pointr)
         ...
         do 34 j=1,npick
            do 344 k=1,3
               norm1 = norm1 + pmn(k,j)*pmn(k,j)
               norm2 = norm2 + prn(k,j)*prn(k,j)
                  dp =    dp + pmn(k,j)*prn(k,j)
 344        continue
 34      continue
         dp = dp / sqrt ( norm1 * norm2 )
         write(utrc,*) dp
         write(*,*) 'FP STOP (TRC)'
         stop
      end if
...
c     adjust pre-equilibration temperature
      ...
      if ( orth ) then
         ...
         if ( nmlst .eq. nomlst ) then
            write(*,*) 'You have requested NO FORWARD first passage'
         end if
         if ( pmlst .eq. nomlst ) then
            write(*,*) 'You have requested NO BACKWARD first passage'
         end if
      else
         write(*,*) "NO ORTH, NO FP"
      end if
...
c     check that required files were opened
      ...
      else if (orth .and. .not.fopen(upfr)) then
         level = 1
         call alert(name,namel,'upfrc not opened',16,level)

      else if (tfp .and. .not.fopen(upvl)) then
         level = 1
         call alert(name,namel,'upvl not opened',16,level)
...
c     MIDDLE (current) MILESTONE. Normalized version is in
c     unitgrad. (norm1 is unused)
      call getmlst(umlst,pmp,pmn,tmlst,num_mlsts,npick,pointr)
